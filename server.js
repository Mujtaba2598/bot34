...
app.get('/health', (req, res) => {
    res.json({ status: 'ok', message: 'Rate Limit Protected Bot' });
});
...
// RATE-LIMIT PROTECTED BINANCE API
class BinanceAPI {
    static endpoints = {
        base: [
    ...
    // RATE LIMIT PROTECTION: Minimum 1.5 seconds between ALL API calls
    static async rateLimitDelay() {
        const now = Date.now();
        const timeSinceLastRequest = now - rateLimit.lastRequestTime;
        const minDelay = 1500; // 1.5 seconds minimum between ALL calls
    ...
    // ORDER RATE LIMIT: Minimum 200ms between orders
    static async orderRateLimit() {
        const now = Date.now();
        const timeSinceLastOrder = now - (rateLimit.lastOrderTime || 0);
        const minOrderDelay = 200; // 200ms minimum between orders
    ...
    static async makeRequest(endpoint, method, apiKey, secret, params = {}, useTestnet = false) {
        try {
            // RATE LIMIT PROTECTION: Delay before EVERY request
            ...
            // Add recvWindow to handle time drift
            const queryParams = { 
                ...params, 
            ...
            for (const baseUrl of endpointsToTry) {
                try {
                    ...
                    if (usedWeight) {
                        ...
                        rateLimit.totalWeightUsed = weight;
    ...
    static async placeMarketOrder(apiKey, secret, symbol, side, quoteOrderQty, useTestnet = false) {
        try {
            // ORDER RATE LIMIT: Extra delay for orders
...
app.listen(PORT, '0.0.0.0', () => {
    console.log('\n' + '='.repeat(60));
    console.log('🌙 HALAL AI TRADING BOT - RATE LIMIT PROTECTED VERSION');
